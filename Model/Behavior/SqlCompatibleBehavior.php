 ...
 */
    ...
    // Toma los belongsTo
		...
		if( !empty( $associations) && isset( $results [0][$Model->alias]))
		{
		  foreach( $results as $key => $result)
		  {
		    foreach( $associations as $_model => $association)
    		{
    		  if( isset( $result [$Model->alias][$association ['foreignKey']]))
    		  {
    		    $_result = $Model->$_model->find( 'first', array(
      		      'conditions' => array(
      		          $Model->$_model->alias .'.'. $Model->$_model->primaryKey => $result [$Model->alias][$association ['foreignKey']]
      		      )
      		  ));
    		  }
